<script>
	$(document).ready(function() {
		...
		$(".simpan").click(function(){
			window.location="<?php echo site_url('statuskawin/formstatuskawin');?>";
			return false;
		});
		$(".ubah").click(function(){
			var id= $(".bg-gray").attr("href");
			window.location="<?php echo site_url('statuskawin/formstatuskawin');?>/"+id;
			return false;
		});
		...
		$(".ya").click(function(){
			var id=$(".bg-gray").attr("href");
			window.location="<?php echo site_url('statuskawin/hapusstatuskawin');?>/"+id;
		});
		$(".reset").click(function(){
			window.location="<?php echo site_url('statuskawin');?>";
		});
	});
</script>
<?php
...
<div class="row">
	<div class="col-md-12">
		<div class="box box-primary">
			<?php echo form_open('statuskawin',array("id"=>"formsubmit"));?>
					...
					<tbody>
						<?php
		                    ...
		                    foreach ($row->result() as $row){
		                        $i++;
		                        echo "<tr id='data' href='".$row->id_statuskawin."'>
		        						 <td class='text-right'>".$i."</td>
		                                 <td>".$row->nama_statuskawin."</td>		                                 
		                              </tr>";
		                    }
		                    for ($n=0;$n<=(30-$i);$i++){
		                    	echo 
